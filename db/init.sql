CREATE TABLE alpha_candles_15m (
            ...
            volume REAL, quote_vol REAL, trades INTEGER, taker_buy_quote_vol REAL, source_env TEXT NOT NULL DEFAULT 'mainnet', is_closed INTEGER NOT NULL DEFAULT 1,
...
CREATE TABLE account_live_balances (
...
CREATE TABLE account_live_positions (
...
CREATE INDEX idx_account_live_positions_account
            ON account_live_positions(account_id, symbol);
CREATE TABLE account_live_orders (
            account_id INTEGER NOT NULL REFERENCES trading_accounts(id) ON DELETE CASCADE,
...
CREATE INDEX idx_account_live_orders_account_symbol
            ON account_live_orders(account_id, symbol);
CREATE TABLE account_stream_state (
            account_id INTEGER PRIMARY KEY REFERENCES trading_accounts(id) ON DELETE CASCADE,
...
CREATE INDEX idx_strategy_decisions_account_run_filter ON strategy_decisions(account_id, run_id, filter_reason);
CREATE INDEX idx_strategy_decisions_account_run_id ON strategy_decisions(account_id, run_id, id DESC);
CREATE INDEX idx_strategy_decisions_account_time ON strategy_decisions(account_id, time DESC, id DESC);
...
CREATE INDEX idx_strategy_decisions_account_symbol_stage_id
            ON strategy_decisions(account_id, symbol, decision_stage, id DESC);
CREATE INDEX idx_alpha_scan_futures_time ON alpha_scan_scores(futures_symbol, time DESC);
...
CREATE INDEX idx_fc1h_env_sym_time ON futures_candles_1h(source_env, symbol, time DESC);
CREATE INDEX idx_fc15m_env_sym_time ON futures_candles_15m(source_env, symbol, time DESC);
CREATE INDEX idx_fc6h_env_sym_time ON futures_candles_6h(source_env, symbol, time DESC);
CREATE INDEX idx_fc24h_env_sym_time ON futures_candles_24h(source_env, symbol, time DESC);
CREATE INDEX idx_fut_time_symbol ON futures_data(time DESC, symbol);
CREATE INDEX idx_fut_sym ON futures_data(symbol, time);
CREATE INDEX idx_fut_env_symbol_time ON futures_data(source_env, symbol, time DESC);
...
CREATE INDEX idx_service_runtime_health
...
CREATE TABLE futures_candles_1m (
            time TEXT NOT NULL,
            symbol TEXT NOT NULL,
            open REAL NOT NULL,
            high REAL NOT NULL,
...
CREATE TABLE alpha_square_posts (
...
CREATE INDEX idx_alpha_square_posts_asset_time
            ON alpha_square_posts(base_asset, published_at DESC);
CREATE TABLE alpha_square_sentiment_snapshots (
            time TEXT NOT NULL,
            base_asset TEXT NOT NULL,
...
CREATE INDEX idx_alpha_square_sentiment_asset_time
            ON alpha_square_sentiment_snapshots(base_asset, time DESC);
CREATE TABLE alpha_candles_1m (
            time TEXT NOT NULL,
            alpha_symbol TEXT NOT NULL,
            open REAL NOT NULL,
            high REAL NOT NULL,
...
CREATE TABLE candle_sync_runtime (
...
CREATE INDEX idx_c1m_env_sym_time ON candles_1m(source_env, symbol, time DESC);
CREATE INDEX idx_fc1m_env_sym_time ON futures_candles_1m(source_env, symbol, time DESC);
CREATE INDEX idx_ac1m_env_sym_time ON alpha_candles_1m(source_env, alpha_symbol, time DESC);
CREATE INDEX idx_agg_candles_lookup ON aggregated_candles(market_kind, source_env, symbol, interval, time DESC);
CREATE INDEX idx_agg_candles_complete ON aggregated_candles(interval, is_complete, time DESC);
CREATE INDEX idx_candle_gaps_status ON candle_gaps(status, detected_at);
CREATE INDEX idx_candle_runtime_market ON candle_sync_runtime(market_kind, source_env);
CREATE INDEX idx_decision_actions_source_decision ON decision_actions(source_decision_id);
CREATE INDEX idx_decision_actions_source_trade ON decision_actions(source_trade_id);
CREATE INDEX idx_decision_outcomes_complete_time ON decision_outcomes(is_complete, signal_time);
