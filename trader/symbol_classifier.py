...
def classify_symbol(row: Any, v3_signals: dict | None = None, side: str | None = None) -> dict:
    ...
    c["score"] = _score_item(c["score"], c["reasons"], 24 if ret_24h <= -0.04 else 0, "跌幅释放")
    c["score"] = _score_item(c["score"], c["reasons"], 18 if not is_down and rs >= 55 else 0, "不是明显弱势")
    c["score"] = _score_item(c["score"], c["reasons"], 18 if bid_support else 0, "盘口有承接")
    c["score"] = _score_item(c["score"], c["reasons"], 14 if funding <= 0.001 else 0, "资金费率不过热")
    c["score"] = _score_item(c["score"], c["reasons"], 12 if is_low else 0, "接近低位/支撑")
